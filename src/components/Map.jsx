...
export default function Map(props) {
  ...
  useEffect(() => {
    // this is really aids so bear with me
    if (props.locations) {
      let newLocations = []; // create an empty array to add all the new geoJSON stuff
      props.locations.forEach((row) => {
        // for every bike rack, create a geoJSON format object and add it to the newLocations arr
  ...
  useEffect(() => {
    if (map.current.getSource("locations")) {
      //update the source of the bike racks on the map
